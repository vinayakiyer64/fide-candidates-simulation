...
from src.allocation.rating_fallback import RatingFallbackAllocation
...
class QualificationSimulator:
    ...
    def simulate_one_season(self) -> List[Player]:
        ...
        for slot in self.config.slots:
            tournament = self._create_tournament(slot.tournament_type, **slot.kwargs)
            # Get deep standings (top 20) for allocation flexibility
            standings_map[slot.tournament_type] = tournament.get_standings(top_n=20)
        ...
        # 3. Rating Fallback: Fill remaining spots from rating list
        remaining_spots = self.config.target_candidates - len(final_qualifiers)
        if remaining_spots > 0:
            rating_strategy = RatingFallbackAllocation(self.players)
            rating_qualifiers = rating_strategy.allocate(
                standings=[],  # Not used by RatingFallbackAllocation
                max_spots=remaining_spots,
                already_qualified=qualified_ids
            )
            final_qualifiers.extend(rating_qualifiers)
